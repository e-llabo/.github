...
### 3. Slack用テキスト整形（Claude整形ステップ）

スクリプトがHTMLと同時に生成するJSONファイル（`~/reports/daily-report-YYYY-MM-DD.json`）を読み込み、以下のフォーマットでSlack用テキストを整形してユーザーに提示する。

**整形ルール:**

1. JSONの `project_items` と `activities` を `project_name`（Project名）でグルーピングする
2. 各Project内で、`parent` がある子課題は親課題の下にツリー表示する
   - 親課題行: `{親課題タイトル} ([#{番号}](URL))`
   - 子課題行: `- {子課題タイトル} ([#{番号}](URL)) → {ステータス}`
3. 親課題がない単独アイテムは「その他」にまとめて `- {タイトル} ([#{番号}](URL)) → {ステータス}` で表示する
4. ステータス表記:
   - `state: CLOSED` or コメントで「完了確認・クローズ」→ `→完了`
   - `status: In progress` → `→進行中`
   - `status: In review` → `→レビュー中`
   - `blocked: Yes` → `→ブロック中`
   - それ以外 → ステータスをそのまま表示
5. `activities` のPR（マージ・レビュー等）は、そのPRが属するリポのProjectに紐づけて表示する。Projectが不明ならリポ名で表示
6. 本日活動があったアイテムのみ表示する（Projectの全アイテムを列挙しない）
...
{親課題タイトル} ([#{番号}](URL))
- {子課題タイトル} ([#{番号}](URL)) → {ステータス}
- {子課題タイトル2} ([#{番号}](URL)) → {ステータス}
...
**重要:**
- Issue番号は必ずMarkdownリンク付きにする: `([#番号](https://github.com/{org}/{repo}/issues/{番号}))`
- PRの場合は `/pull/{番号}` を使う
- Issue番号はタイトルの後ろに付ける（`#番号 タイトル` ではなく `タイトル ([#番号](URL))` ）
- 本日活動があったアイテム（`activities` に含まれる）を優先的に表示する
- PRのマージやレビューも対応するIssueと紐づけて表示する
- Project名がない場合はリポジトリ名をカテゴリとして使う

### 4. 結果の確認

生成されたHTMLファイルのパスをユーザーに伝える。ブラウザで開けない環境の場合はファイルパスを表示する。
整形したSlack用テキストをユーザーに提示し、追記・修正の有無を確認する。
